...
//Post route for submitting post

router.post('/api/submitpost/', (req, res, next) => {
  
  Post.findOne( {post: req.body.post})
  .then((foundPost) => {
    //If the Post has already been sent, send error status and message
    if(foundPost) {
      console.log("This post already exists")
      return res.status(403).send("Error: This post already exists");
    } else {
      new Post({
        user: req.body.user,
        post: req.body.post,
